...
exports.processRegister=(reqBody)=>{
    ...
    let{lastName,emailAddress,organizationName,firstName,mobileNumber,password}=reqBody
    a.push({
        firstName:firstName,
        lastName:lastName,
...
exports.checkNameRules=(reqBody)=>{
    const{mobileNumber,lastName,emailAddress,organizationName}=reqBody
    var a=readFromFile("/bookwormUsers.json")
    var result;
    if(a.length>0){
        a.forEach(o=>{
            o.lastName==lastName?result={type:"error",message:"Lastname Exists"}:result=true
